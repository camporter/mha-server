...
public class ID3v2PictureFrameData extends AbstractID3v2FrameData {
	...
	public ID3v2PictureFrameData(boolean unsynchronisation, String mimeType, byte pictureType,
	...
	protected byte[] packFrameData() {
		...
		if (description != null && description.toBytes().length > 0) {
			...
			BufferTools.copyIntoByteBuffer(description.toBytes(), 0, descriptionLength, bytes,
					mimeTypeLength + 3);
		}
		bytes[mimeTypeLength + descriptionLength + 3] = 0;
		if (imageData != null && imageData.length > 0) {
			BufferTools.copyIntoByteBuffer(imageData, 0, imageData.length, bytes, mimeTypeLength
					+ descriptionLength + 4);
		}
		return bytes;
	}
	...
	protected int getLength() {
		...
		if (description != null)
			length += description.toBytes().length;
		if (imageData != null)
