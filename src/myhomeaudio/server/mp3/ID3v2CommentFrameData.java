...
public class ID3v2CommentFrameData extends AbstractID3v2FrameData {
	...
	public ID3v2CommentFrameData(boolean unsynchronisation, String language,
			EncodedText description, EncodedText comment) {
		super(unsynchronisation);
	...
	protected byte[] packFrameData() {
		...
		if (comment != null && comment.toBytes().length > 0) {
			BufferTools.copyIntoByteBuffer(comment.toBytes(), 0, comment.toBytes().length, bytes,
					descriptionLength + 5);
		}
		return bytes;
	}
