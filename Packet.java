...
public class Packet {
    ...
    public Packet(int Framenb, byte[] data, int dataLen) {
        ...
        header[0] = new Integer(SeqNum >> 8).byteValue();
        header[1] = new Integer(SeqNum).byteValue();
    ...
    public Packet(byte[] packet, int packet_size) {
        ...
        if (packet_size >= HEADER_SIZE) {
            ...
            SeqNum = unsigned_int(header[1]) + 256 * unsigned_int(header[0]);
        }
    }

    //getpayload: return the payload bitstream of the packet and its size
